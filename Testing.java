// This Java program processes CLAS12 HIPO files to analyze electron-proton scattering data
...
public class Testing extends ... {
    public static void main(String[] args) {
        ...
        hNu.setTitle("(v)nu Distribution");
        ...
        H1F hElectronVz = new H1F("hElectronVz", 100, -15, 10);
        ...
        H1F hProtonVz = new H1F("hProtonVz", 100, -15, 10);
        ...
        // Histogram for vz difference (electron - proton)
        H1F hVzDiff = new H1F("hVzDiff", 100, -10, 10);
        hVzDiff.setTitle("Electron-Proton v_z Difference");
        hVzDiff.setTitleX("v_z^e - v_z^p  [cm]");
        hVzDiff.setTitleY("Counts");
        hVzDiff.setFillColor(5);
        hVzDiff.setLineColor(1);

        // 2D Histogram: Number of protons vs z
        H2F hProtonNumVsVz = new H2F("hProtonNumVsVz", 100, -10, 0, 10, 0, 10);
        hProtonNumVsVz.setTitle("Number of Protons vs z");
        hProtonNumVsVz.setTitleX("Proton v_z [cm]");
        hProtonNumVsVz.setTitleY("Number of Protons");
            ...
            if (particles.getRows() > 0) {
                for (int i = 0; i < particles.getRows(); i++) {
                    ...
                    if (pid == 11) { // Electron
                        ...
                        // --- Target selection cut: adjust vz range as needed ---
                        // select liquid deuterium target region
                        if (vz > -8 && vz < -2) {
                            hasProtonInTarget = true;
                        }
                        // For solid target, use e.g.: if (vz > 0) { hasProtonInTarget = true; }
                        //if (vz > 0) {
                        //    hasProtonInTarget = true;
                        //}
                    }
                ...
                // Only fill histograms if event passes the target cut (has a proton in the selected vz region)
                if (hasProtonInTarget) {
                    ...
                    // Fill vz difference histogram if at least one electron and one proton
                    if (electronVzs.size() > 0 && protonVzs.size() > 0) {
                        double vzDiff = electronVzs.get(0) - protonVzs.get(0);
                        hVzDiff.fill(vzDiff);
                    }

                    // Fill 2D histogram: number of protons vs each proton vz in this event
                    for (double vz : protonVzs) {
                        hProtonNumVsVz.fill(vz, protonCount);
                    }
                }
            }
        }
        ...
        // Canvas for number of protons vs z
        TCanvas protonNumVsVzCanvas = new TCanvas("ProtonNumVsVz", 800, 600);
        protonNumVsVzCanvas.draw(hProtonNumVsVz);
        protonNumVsVzCanvas.save("/home/reh1/Pictures/LiquidD_DIS_ProtonNumVsVz.png");
